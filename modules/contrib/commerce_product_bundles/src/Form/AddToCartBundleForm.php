...
/**
 * Provides the order item add to cart bundle form.
 *
 * Code was taken from and modified:
 * @see \Drupal\commerce_cart\Form\AddToCartForm
 *
 * Could not extend AddToCartForm because of form save.
 */
class AddToCartBundleForm extends ContentEntityForm implements AddToCartFormInterface {
  ...
  /**
   * AddToCartBundleForm constructor.
   *
  ...
  public function __construct(EntityRepositoryInterface $entity_repository, EntityTypeBundleInfoInterface $entity_type_bundle_info,
                              TimeInterface $time, CartManagerInterface $cart_manager, CartProviderInterface $cart_provider, OrderTypeResolverInterface $order_type_resolver,
                              CurrentStoreInterface $current_store, ChainPriceResolverInterface $chain_price_resolver, AccountInterface $current_user) {
  ...
  /**
   * {@inheritdoc}
   * @see \Drupal\commerce_cart\Form\AddToCartForm::actions()
   */
  ...
  /**
   * {@inheritdoc}
   * @see \Drupal\commerce_cart\Form\AddToCartForm::setEntity()
   */
  ...
  /**
   * {@inheritdoc}
   * @see \Drupal\commerce_cart\Form\AddToCartForm::submitForm()
   */
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // Set ref. PV and quantity on bundle order item.
    $selected_bundle_variations = reset($form_state->getValue('purchased_entity'));
    $ref_eck = [];
    if(isset($selected_bundle_variations['bundle_variations_ref_options'])){
      foreach ($selected_bundle_variations['bundle_variations_ref_options'] as $product_id => $selected_bundle_variation){
        $ref_eck[] =  [
          'product_var_id' => $selected_bundle_variation,
          'quantity' => isset($selected_bundle_variations['quantity'][$product_id]) ?  $selected_bundle_variations['quantity'][$product_id] : 1
        ];
      }
    }
    $order_item->set('field_product_variation_ref', $ref_eck);
  ...
  /**
   * {@inheritdoc}
   * @see \Drupal\commerce_cart\Form\AddToCartForm::buildEntity()
   */
