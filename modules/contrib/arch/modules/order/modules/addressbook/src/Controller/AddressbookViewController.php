...
/**
 * Defines a controller to render a single addressbookitem.
 */
class AddressbookViewController extends EntityViewController {
  ...
  /**
   * Creates an AddressbookViewController object.
   *
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $addressbookitem, $view_mode = 'full', $langcode = NULL) {
    /** @var \Drupal\arch_addressbook\AddressbookitemInterface $addressbookitem */
    $build = $this->entityTypeManager
      ->getViewBuilder($addressbookitem->getEntityTypeId())
      ->view($addressbookitem, $view_mode);

    $build['#pre_render'][] = [$this, 'buildTitle'];
    $build['#entity_type'] = $addressbookitem->getEntityTypeId();
    $build['#' . $build['#entity_type']] = $addressbookitem;

    foreach ($addressbookitem->uriRelationships() as $rel) {
      $url = $addressbookitem->toUrl($rel);
      ...
      if ($this->currentUser->isAuthenticated() || $url->access($this->currentUser)) {
        // Set the addressbookitem path as the canonical URL to prevent
        // duplicate content.
  ...
  /**
   * The _title_callback for the page that renders a single addressbookitem.
   *
   * @param \Drupal\Core\Entity\EntityInterface $addressbookitem
   *   The current addressbookitem.
   *
   * @return string
   *   The page title.
   */
  public function title(EntityInterface $addressbookitem) {
    return $this->entityRepository->getTranslationFromContext($addressbookitem)->label();
  }
