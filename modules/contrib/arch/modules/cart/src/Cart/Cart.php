...
class Cart implements CartInterface {
  ...
  protected function buildTotal(array $items, $currency = NULL) {
    ...
    // Since we use 'custom' VAT category, which has 0 rate, we have to empty
    // the vat_amount and then make 'net' equal with 'gross' if 'base' was set
    // to 'gross', or, in the opposite direction if 'base' is set to 'net'.
    if ($values['vat_rate'] === 0) {
      $values['vat_amount'] = 0;

      if ($values['base'] == self::PRICE_TYPE_NET) {
        $values['gross'] = $values['net'];
      }
      else {
        $values['net'] = $values['gross'];
      }
    }
  ...
  protected function buildPriceInstance(array $values) {
    ...
    if (!empty($values['net'])) {
      $values['vat_rate'] = round(($values['gross'] / $values['net']) - 1, 2);
    }
