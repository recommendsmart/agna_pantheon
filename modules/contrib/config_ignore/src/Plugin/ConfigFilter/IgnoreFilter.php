...
class IgnoreFilter extends ConfigFilterBase implements ContainerFactoryPluginInterface {
  ...
  protected function matchConfigName($config_name) {
    ...
    // Check if the entire collection should be ignored.
  ...
  /**
   * Returns true if the entire current collection should be ignored
   *
   * @return bool
   */
  protected function ignoreCollection() {
    // The collection name we are currently in.
    $collection_name = $this->getFilteredStorage()->getCollectionName();

    // We don't support ignoring the default collection.
    if (empty($collection_name)) {
      return FALSE;
    }

    // If the collection is an excluded collection, don't ignore it.
    if (in_array(static::COLLECTION_PREFIX . static::FORCE_EXCLUSION_PREFIX . $collection_name, $this->configuration['ignored'], TRUE)) {
      return FALSE;
    }

    foreach ($this->configuration['ignored'] as $config_name) {
      // Check if $config_name in ignored isn't requesting an ignore of an entire
      // collection.
      $collections = explode(static::COLLECTION_PREFIX, $config_name);
      if (isset($collections[1]) && fnmatch($collections[1], $collection_name)) {
        return TRUE;
      }
    }
    return FALSE;
  ...
  protected function activeRead($name, $data) {
    ...
    if ($this->ignoreCollection()) {
      // If the entire collection should be ignored, read from active storage.
