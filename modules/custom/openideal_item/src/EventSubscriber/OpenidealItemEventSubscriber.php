...
namespace Drupal\openideal_item\EventSubscriber;
...
/**
 * Class OpenidealItemEventSubscriber.
 */
class OpenidealItemEventSubscriber implements EventSubscriberInterface {
  ...
  public function onContentStateChange(ContentModerationStateChangedEvent $event) {
    ...
    if (($event->getOriginalState() === 'draft' || $event->getOriginalState() === FALSE)
      ...
      $message = $openideal_config->field_item_approval_message->view('default');
      if (!empty($message)) {
