...
/**
 * @file
 * Contains \Drupal\burndown\Services\ProjectNavBlock.
 */
...
class ProjectNavBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * @var CurrentPathStack $path_service.
   */
  ...
  public function getBoard() {
    ...
    else if (strpos($current_path, 'completed') !== FALSE) {
      $board = 'completed';
    }
    else {
      $board = 'backlog';
  ...
  public function getCacheTags() {
    if ($project = \Drupal::routeMatch()->getParameter('shortcode')) {
      return Cache::mergeTags(parent::getCacheTags(), array('project:' . $project));
    }
    else {
