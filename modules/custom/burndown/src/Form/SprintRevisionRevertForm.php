...
class SprintRevisionRevertForm extends ConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Sprint: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Sprint %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
