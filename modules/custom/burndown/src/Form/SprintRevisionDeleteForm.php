...
class SprintRevisionDeleteForm extends ConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Sprint: deleted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Revision from %revision-date of Sprint %title has been deleted.', ['%revision-date' => format_date($this->revision->getRevisionCreationTime()), '%title' => $this->revision->label()]));
