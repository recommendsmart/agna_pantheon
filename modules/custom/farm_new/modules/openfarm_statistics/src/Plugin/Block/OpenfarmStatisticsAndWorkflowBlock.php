...
/**
 * Provides a 'OpenfarmStatisticsAndWorkflowBlock' block.
 *
 * @Block(
 ...
 * )
 */
class OpenfarmStatisticsAndWorkflowBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if (isset($contexts['node'])
      ...
      $node = $contexts['node'];
      $statistics_block = $this->blockManager->createInstance('openfarm_statistics_record_statistics');
      $statistics_block->setContext('node', $node);
      $statistics_block->setContext('view_mode', $contexts['view_mode']);
      $status = $this->blockManager->createInstance('openfarm_statistics_status');
      $status->setContext('node', $node);
      $build = [
        '#type' => 'container',
        '#attributes' => ['class' => ['record-statistics-and-status-block']],
