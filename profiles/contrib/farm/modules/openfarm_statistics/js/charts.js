(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.OpenfarmStatisticsC3EntityByCharts = {
    attach: function (context, settings) {
      // @Todo: combine this and OpenfarmStatisticsC3PerDayCharts behavior?
      let chartsSettings = settings.charts.byEntity;
      for (var id in chartsSettings) {
        if (chartsSettings.hasOwnProperty(id)) {
          $(chartsSettings[id].bindTo, context).once('openfarm_statistics_c3_entity_by_charts').each(function () {
            c3.generate({
              bindto: $(this).get(0),
              padding: {
                left: 30,
   ...
   */
  Drupal.behaviors.OpenfarmStatisticsC3PerDayCharts = {
    attach: function (context, settings) {
      let chartsSettings = settings.charts.perDay;
      for (var id in chartsSettings) {
        if (chartsSettings.hasOwnProperty(id)) {
          $(chartsSettings[id].bindTo, context).once('openfarm_statistics_c3_per_day_charts').each(function () {
            c3.generate({
              bindto: $(this).get(0),
              padding: {
                left: 30,
   ...
   */
  Drupal.behaviors.OpenfarmStatisticsCharts = {
    attach: function (context, settings) {
      $('.charts').once('openfarm_statistics_charts', context).each(function (name) {
        ...
        // @see Drupal\openfarm_statistics\Plugin\Block\Charts
