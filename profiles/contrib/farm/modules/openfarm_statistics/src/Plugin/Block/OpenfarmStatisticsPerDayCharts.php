...
/**
 * Provides a 'OpenfarmStatisticsByCategoryCharts' block.
 *
 * @Block(
 ...
 */
class OpenfarmStatisticsPerDayCharts extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $build['#attached']['drupalSettings']['charts']['perDay'][$entity] = [
      ...
      'label' => $this->t("No. of @entity", ['@entity' => ($entity != 'node' ? $entity : 'records') . 's']),
      'max' => $this->max,
    ];
    $build['#cache']['tags'] = [$entity . '_list' . ($entity == 'node' ? ':record' : '')];
  ...
  protected function getData() {
    ...
    if ($filters = $this->getFilters()) {
      $to = $filters[OpenfarmStatisticsDateSelectForm::TO] ?? $to;
      $from = $filters[OpenfarmStatisticsDateSelectForm::FROM] ?? $from;
    }
    ...
    if ($entity == 'node') {
      $select->condition('d.type', 'record');
    }
