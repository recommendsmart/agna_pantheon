...
(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.openfarmThemeTeaser = {
    attach: function (context, settings) {
      $('.teaser-top-section--section__first').once('openfarm_theme_teaser').each(function () {
        var $this = $(this);
   ...
   */
  Drupal.behaviors.openfarmCustomSelectOption = {
    attach: function (context, settings) {
      $('.custom-sort', context).once('openfarm_custom_select_option').each(function () {
   ...
   */
  Drupal.behaviors.openfarmThemeNavigationToggle = {
    attach: function (context, settings) {
      $('.site-navigation--dismiss, .navigation-overlay, .site-sidebar--dismiss').once('openfarm-theme-navigation-toggle-overlay').on('click', function () {
        // hide sidebar
      ...
      $('#sidebar-collapse').once('openfarm-theme-navigation-toggle').on('click', function () {
        // open sidebar
      ...
      $('#site-notification-collapse').once('openfarm-theme-navigation-toggle').on('click', function () {
        // open sidebar
   ...
   */
  Drupal.behaviors.openfarmThemeCommentsFormAnimation = {
    attach: function (context, settings) {
      $('.comments--header__add-comment-btn, .site-footer-open-comments-btn', context).once('openfarm-theme-comments-form-animation').on('click', function () {
      ...
      $('.comment-form--cancel-btn', context).once('openfarm-theme-comments-form-animation-reply').on('click', function () {
        // hide sidebar
      ...
      $('.ajax-comments-form-edit .comment-form--cancel-btn').once('openfarm-theme-comments-form-animation-edit').addClass('d-none');
    }
  }
   ...
   *   Change label.
   */
  Drupal.behaviors.openfarmThemeLikeWidgetLabel = {
    attach: function (context, settings) {
      $('.votingapi-reaction-form').once('openfarm_theme_like_widget_label').each(function () {
        var $this = $(this);
        ...
        if ($('.radio input', $this).is(':checked')) {
          ...
          if ($label.parents('.holding-voting').length) {
            $label.text(Drupal.t('Like holding'));
          } else if ($label.parents('.region-sidebar').length) {
            $label.text(Drupal.t('Like record'));
   ...
   */
  Drupal.behaviors.openfarmThemeCopyUrlButton = {
    attach: function (context, settings) {
      $('.share-buttons--buttons__copy_url', context).once('openfarm_theme_copy_url_button').each(function () {
        var $this = $(this);
   ...
   */
  Drupal.behaviors.openfarmThemeMobileShareBlock = {
    attach: function (context, settings) {
      $('.site-footer-open-share-btn', context).once('openfarm_theme_mobile_share_block').on('click', function () {
  ...
  /**
   * Attach behaviours on exposed filter in records page.
   *
   * @type {Drupal~behavior}
   *
   * @prop {Drupal~behaviorAttach} attach
   ...
   */
  Drupal.behaviors.openfarmThemeExposedIdeasFilter = {
    attach: function (context, settings) {
      $('.teaser-view-mode .views-exposed-form fieldset, .view--user-admin-people--community-page .views-exposed-form fieldset', context).once('openfarm_theme_exposed_records_filter').each(function () {
        var $this = $(this);
   ...
   */
  Drupal.behaviors.openfarmThemeUserProfile = {
    attach: function (context, settings) {
      $('.user-profile-section-top .user-profile-section-top--section__second .field__label').once('openfarm_theme_user_profile').each(function () {
        var $this = $(this);
        var text = $this.text();
   ...
   */
  Drupal.behaviors.openfarmThemeCommentsReply = {
    attach: function (context, settings) {
      $('.comments--thread', context).once('openfarm_theme_comments_last_child').each(function () {
        var $this = $(this);
      ...
      $('.comment-show .single-comment--open-replies', context).once('openfarm_theme_comments_reply').each(function () {
        var $this = $(this);
